'''
This code converts downloading of the records (if found) in function
from mhaPendingCases3.py
Changes: in path.
Skipped mhaPendingCases & ...2 as fed up with alterations
...
@retry(retry=retry_if_exception_type(IOError), wait=wait_fixed(50))
def might_io_error():
...
def district():
    ...
    def complex_and_act():
        ...
        while courtComp < courtComplexLen:
            ...
            actDropDown = Select(driver.find_element_by_css_selector('#actcode'))
            while len(actDropDown.options) < 1:
                time.sleep(1)
            else:
                actDropDown.select_by_value('33')
    ...
    while i <= lenOpts:
        try:
            ...
            driver.refresh()
            return print("time out exception " + str(dateToday))
        except WebDriverException:
            time.sleep(120)
            driver.refresh()
            return print("Connectivity Issues found " + str(dateToday))
    else:
