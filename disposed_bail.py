"""from disposed_case_status_2020. Change = FireFox to Chrome"""
...
from set_driver import chrome_driver

# import from set_driver.py and set the driver.
driver = chrome_driver()
# constants
district_name_here = sys.argv[1]
URL = 'https://districts.ecourts.gov.in/' + district_name_here

main_Directory_name = sys.argv[1]

base_Directory = os.path.join(
    '/home/sangharshmanuski/Documents/ecourts_bail_disposed/2020',
    main_Directory_name)
main_Directory_name_sub = sys.argv[2]
...
def view(some_complex=None):
    ...
    start_lenth = sys.argv[3]
    end_lenth = sys.argv[4] if int(sys.argv[4]) >= int("3") else len(list_all_view)
...
start_number = sys.argv[5] if len(sys.argv) >= 6 else "0"
i = int(start_number)
while i < len(this_name_complex_list):
    try:
        ...
        # create file with headers for writing details of case.
